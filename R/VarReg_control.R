...
#' @param epsilon  Positive convergence tolerance. If \eqn{\theta} is a vector of estimates, convergence is assumed when \eqn{\sqrt{(\sum (\theta^{old} - \theta_{new})^2)}\sqrt{\sum (\theta_{old})^2} }. This should be smaller than \code{bound.tol}.
